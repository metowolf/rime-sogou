class Parse {
  ...
  _readInt(begin) {
    return this.buffer.readInt32LE(begin)
  }
  ...
  _table() {
    ...
    while (pos < length) {
      ...
      // 索引
      let index = this._readShort(pos)
      pos += 2

      // 音節長度
      let word_len = this._readShort(pos)
      pos += 2
  ...
  chinese() {
    ...
    while (pos < length) {
      ...
      // 同音詞
      let homophone = this._readShort(pos)
      pos += 2
      ...
      for (let i = pos; i < pos + index; i += 2) {
        pinyin.push(this.table[this._readShort(i)])
      }
      pos += index
      ...
      for (let i = 0; i < homophone; i += 1) {
        ...
        // 擴展數據長度
        let ext_len = this._readShort(pos)
        pos += 2
